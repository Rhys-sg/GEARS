...
class GEARS:
    ...
    def predict(self, pert_list, gene_subset=None):
        """
        ...
        for pert in pert_list:
            ...
            pred_values = np.mean(p.detach().cpu().numpy(), axis=0)

            if gene_subset is not None:
                gene_indices = [self.gene_list.index(g) for g in gene_subset if g in self.gene_list]
                pred_values = pred_values[gene_indices]

            results_pred['_'.join(pert)] = pred_values
                
        self.saved_pred.update(results_pred)
        
        if self.config['uncertainty']:
            logvar_values = np.mean(unc.detach().cpu().numpy(), axis=0)
            if gene_subset is not None:
                logvar_values = logvar_values[gene_indices]
            results_logvar['_'.join(pert)] = logvar_values
            results_logvar_sum['_'.join(pert)] = np.exp(-np.mean(results_logvar['_'.join(pert)]))
        else:
            return results_pred
    ...
    def train(self, epochs = 20, 
        ...
        for epoch in range(epochs):
            ...
            train_res = evaluate(train_loader, self.model, self.config['uncertainty'], self.device, gene_indices)
            val_res = evaluate(val_loader, self.model, self.config['uncertainty'], self.device, gene_indices)
            train_metrics, _ = compute_metrics(train_res, gene_indices)
            val_metrics, _ = compute_metrics(val_res, gene_indices)
        ...
        test_res = evaluate(test_loader, self.best_model, self.config['uncertainty'], self.device, gene_indices)
        test_metrics, test_pert_res = compute_metrics(test_res, gene_indices)
